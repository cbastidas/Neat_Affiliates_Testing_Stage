...
/**
 * IMPORTANT:
 * Make sure you have this row in Supabase table "terms":
 * slug = "jackburst-terms-of-use"
 * content = "<html>...</html>"
 */
const JACKBURST_TERMS_SLUG = "jackburst-terms-of-use";
...
function TermsOfUseModal({
  ...
  return (
    <div
      className="fixed top-10 left-0 right-0 bottom-0 bg-black bg-opacity-60 flex justify-center items-center z-50"
      onClick={(e) => {
...
const JackburstSignupModal: React.FC<Props> = ({ isOpen, onClose }) => {
  ...
  // Terms modal state (opens only when user clicks Terms of Use)
  const [isTermsOpen, setIsTermsOpen] = useState(false);
  const [termsSlug, setTermsSlug] = useState<string | null>(null);
  const [termsTitle, setTermsTitle] = useState<string>("Terms of Use — Jackburst");
  ...
  // Open Jackburst Terms directly (no chooser for Jackburst)
  const handleTermsClick = () => {
    setTermsSlug(JACKBURST_TERMS_SLUG);
    setTermsTitle("Terms of Use — Jackburst");
    setIsTermsOpen(true);
  };
      ...
      >
        ...
        <form className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="flex flex-col">
            ...
            <input type="text" className="border rounded p-2 mt-1 text-black font-semibold" />
          </div>
          ...
          <div className="flex flex-col">
            ...
            <input type="password" className="border rounded p-2 mt-1 text-black font-semibold" />
          </div>

          {/* Confirm Password */}
          <div className="flex flex-col">
            <label className="font-bold text-black">* Confirm password</label>
            <input type="password" className="border rounded p-2 text-black font-semibold" />
          </div>

          {/* Email Address */}
          <div className="flex flex-col">
            <label className="font-bold text-black">* Email address</label>
            <input type="email" className="border rounded p-2 text-black font-semibold" />
          </div>

          {/* Newsletter */}
          <div className="flex items-center gap-2 mt-2">
            <input type="checkbox" />
            <label className="font-semibold text-black">Email subscription</label>
          </div>

          {/* Country */}
          <div className="flex flex-col">
            <label className="font-bold text-black">* Country</label>
            <select className="border rounded p-2 text-black font-semibold">
          ...
          <div className="flex flex-col">
            ...
            <input className="border rounded p-2 text-black font-semibold" type="text" />
          </div>
          ...
          <div className="flex flex-col">
            ...
            <input className="border rounded p-2 text-black font-semibold" type="date" />
          </div>

          {/* Address */}
          <div className="flex flex-col md:col-span-2">
            <label className="font-bold text-black">Address</label>
            <textarea className="border rounded p-2 text-black font-semibold" rows={3}></textarea>
          </div>

          {/* Zip Code */}
          <div className="flex flex-col">
            <label className="font-bold text-black">Zip code</label>
            <input className="border rounded p-2 text-black font-semibold" />
          </div>
          ...
          {/* Terms of Use (CLICK -> opens Jackburst terms modal directly) */}
