...
/**
 * Vidavegas Terms slug (based on your DB screenshot)
 * NOTE: Both Vidavegas BR and Vidavegas Latam use the same slug in your table:
 * - vidavegas-terms-of-use
 */
const VIDAVEGAS_TERMS_SLUG = "vidavegas-terms-of-use";
...
const VidavegasLatamSignupModal: React.FC<Props> = ({ isOpen, onClose }) => {
  ...
  const [termsTitle, setTermsTitle] = useState<string>("Terms of Use — Vidavegas Latam");

  // Load Vidavegas Latam logo dynamically
  useEffect(() => {
    if (!isOpen) return;
    ...
    const loadLogo = async () => {
       ...
       * 1) Match by name "Vidavegas Latam"
       * 2) If not found, match by group "Vidavegas - Latam" and take the first logo
       */
      const byName = await supabase
        .from("brands")
        .select("logo_url")
        .eq("name", "Vidavegas Latam")
        .single();

      if (!byName.error && byName.data?.logo_url) {
        setLogo(byName.data.logo_url);
        return;
      }

      const byGroup = await supabase
        .from("brands")
        .select("logo_url, order")
        .eq("group", "Vidavegas - Latam")
        .order("order", { ascending: true })
        .limit(1);

      if (!byGroup.error && byGroup.data?.[0]?.logo_url) {
        setLogo(byGroup.data[0].logo_url);
      }
    };
  ...
  // Open Vidavegas Terms directly (no chooser for Vidavegas)
  const handleTermsClick = () => {
    setTermsSlug(VIDAVEGAS_TERMS_SLUG);
    setTermsTitle("Terms of Use — Vidavegas");
    setIsTermsOpen(true);
  };
        ...
        <form className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="md:col-span-2 flex flex-col w-full">
            ...
            <div className="flex flex-wrap gap-x-6 gap-y-2">
              ...
              <label className="flex items-center gap-2 font-semibold">
                <input type="radio" name="market" /> Social
              </label>
              <label className="flex items-center gap-2 font-semibold">
          ...
          {/* Terms of Use (CLICK -> opens Vidavegas terms modal directly) */}
...
export default VidavegasLatamSignupModal;
